plot.barcode <-
function(x, diagLim = NULL, maxdim = NULL, col = NULL, rotated = FALSE,
         barcode = TRUE, logscale = FALSE, band = NULL, lab.line = 2.2, colorBand = "pink",
  ...
  if (!is.null(maxdim) && (!is.numeric(maxdim) ||
      length(maxdim) != 1 || any(maxdim < 0))) {
    stop("maxdim should be a nonnegative integer")
  }
  ...
  if (!is.logical(logscale)) {
    stop("logscale should be logical")
  }
  ...
  if (!is.null(maxdim)) {
    x <- x[which(x[, 1] <= maxdim), , drop = FALSE]
  }
...
### barcode plot
  if (barcode) {
    ...
    Bmax <- max(max(left[which(left != -Inf)]), max(right[which(right != Inf)]))
    Bmin <- min(min(left[which(left != -Inf)]), min(right[which(right != Inf)]))
    ...
    # replace infinite intervals with finite ones
    right <- replace(right, right==Inf, Bmax+(Bmax-Bmin))
    left <- replace(left, left==-Inf, Bmin-(Bmax-Bmin))
