...
public class Transformation_2 {
	...
	public Transformation_2(double s1, double s2) {
		double[][] array = new double[][]{ {s1, 0., 0.},
							 			   {0., s2, 0.},
							 			   {0., 0., 1.} }; 
		this.m = new Matrix(array);
	}
	...
	public Transformation_2(double theta) {
		...
		double[][] array = new double[][] { { cos, -sin,  0.}, 
										 	{ sin,  cos,  0.},
										 	{  0.,   0.,  1.} };
	...
	public Transformation_2(double theta, Point_2 center) {
		Vector_2 vect = new Vector_2(new Point_2(0,0),center);
		Vector_2 vect_inv = vect.multiplyByScalar(-1);
		Transformation_2 rotate = new Transformation_2(vect);
		
		rotate = rotate.compose(new Transformation_2(theta));
		rotate = rotate.compose(new Transformation_2(vect_inv));
